...
public class Estados {
    ...
    public void GetById() {//Creamos otro metodo que ocuapremos para obtener un solo producto que es obtener por Id
        ...
        idEstado = (int) res.getValueAt(0, 0);//Aqui siempre seria en el renglon cero ¿Porque? porque en la primer linea dentro de el metodoGetById yo me estoy asegurando que esa consulta me retorne un solo registro si o si
    ...
    public boolean Add() {//metodo para agregar
        ...
        String query = "INSERT INTO Estados(nombreEstado) "//sentencia para insertar valores, por lo cual se inserto el nombre, csducidada, stock, idFarmacia y activo
                + "VALUES ('" + nombreEstado + "');";// dentro de paerntesis coloco los valores que le quiero meter, para meter valores String se le pone un apostrofe, para meter valores numericos se le pone tal cual, pero como el primero va hacer el nombre pues es String asi que se le pone solo un apostrofe, depues sigue caducidad ese tambien va con apostrofe, despues sigue stock ese va tal cual sin apostrofe
    ...
    public boolean Delete() {//metodo para borrar
        String query = "DELETE FROM Estados WHERE idEstado = " + idEstado;//Aqui se hizo una cosulta como la anterior pero este es para borrar
    ...
    public DefaultTableModel GetAllModelASC() {
      ...
      public DefaultTableModel GetByNameAll() {
        String query = "SELECT * FROM Estados WHERE nombreEstado='"+nombreEstado+"';";//Es una consulta para obtener un solo registro de la tabla de productos, siempre y cuando el id del producto corresponda al que yo le voy a mandar como parametro; entonces la variable o propiedad idProducto ya esta declarada al inicio de la clase y tambien se encuentra dentro del constructor, entonces esa propiedad la tomo y se la mando como parametro a la consulta
