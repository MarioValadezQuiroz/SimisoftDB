...
public class Categoria {
    ...
    public void setActivo(int activo) {
     ...
     public DefaultTableModel GetAllModel() { //creamos un metodo Obtenemos todos los productos, es llamado GetAllModel en formato Model, este metodo ya con su contenido me retorna el DefaultTableModel de la entidad o clase Producto
        String query = "SELECT * FROM Categorias";//Creamos una query que va hacer la instruccion hacia la base de datos, que va hacer entre comillas "seleccioname from de la tabla productos"
    ...
    public void GetById() {//Creamos otro metodo que ocuapremos para obtener un solo producto que es obtener por Id
        ...
        idCategoria = (int) res.getValueAt(0, 0);//Aqui siempre seria en el renglon cero ¿Porque? porque en la primer linea dentro de el metodoGetById yo me estoy asegurando que esa consulta me retorne un solo registro si o si
    ...
    public boolean Add() {//metodo para agregar
        ...
        String query = "INSERT INTO Categorias(nombreCategoria, activo) "//sentencia para insertar valores, por lo cual se inserto el nombre, csducidada, stock, idFarmacia y activo
    ...
    public boolean Delete() {//metodo para borrar
        String query = "DELETE FROM Categorias WHERE idCategoria = " + idCategoria;//Aqui se hizo una cosulta como la anterior pero este es para borrar
   ...
   public DefaultTableModel GetByNameAll() {
        String query = "SELECT * FROM Categorias WHERE nombreCategoria LIKE '"
                +nombreCategoria+"%';";//Es una consulta para obtener un solo registro de la tabla de productos, siempre y cuando el id del producto corresponda al que yo le voy a mandar como parametro; entonces la variable o propiedad idProducto ya esta declarada al inicio de la clase y tambien se encuentra dentro del constructor, entonces esa propiedad la tomo y se la mando como parametro a la consulta
        return dataAccess.Query(query); //este metodo lo regresa para guardar en un defaultTableModel
    }
